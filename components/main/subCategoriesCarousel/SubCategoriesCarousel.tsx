		...
		<div className={styles.carousel}>
			...
			>
				{cardLinks.map((link, index) => (
					<Link href={`catalog/category/${link.slug}`} key={index} className={styles.carousel_card}>
						<h3>{link.name}</h3>
					</Link>
				))}
			...
			<div className={styles.carousel_slideIndicatorContainer}>
				{[0, -490, -490 * 2].map((num) => (
					<div
						...
						style={{
							backgroundColor: `${
								num === carouselPosition ? "var(--orange-color)" : "var(--transparent-dark-color)"
							}`,
							borderColor: `${
								num === carouselPosition ? "var(--orange-color)" : "var(--foreground-color)"
							}`,
						}}
					/>
				))}
